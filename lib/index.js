...
const getAbsMainFilePath = function(modulePath, file, option) {
    ...
    if (fs.existsSync(mainPath)) {
        ...
        if (stats.isDirectory()) {
            const indexPath = path.resolve(mainPath, 'index.js');
            if (fs.existsSync(indexPath)) {
                return indexPath;
            }
            //is dir but no index.js, or maybe .mjs?
            return '';
        }
    }
...
const getModuleFilePath = function(modulePath, file, option) {
    ...
    if (!absMainPath) {
        Util.addLog(EC.red(`ERROR: Not found main file: ${path.resolve(modulePath, file)}`), option);
    }
    //relative path
...
const getModuleOverrideFiles = function(modulePath, moduleConf, option) {
    ...
    if (!Util.hasOwn(option.overrides, moduleName)) {
        return;
    }
...
const generateModuleInfo = function(moduleInfo, moduleName, modulePath, moduleConf, option) {
    //already done
    if (!option.moduleMap) {
        option.moduleMap = {};
    }
...
const getModuleInfo = function(moduleName, dependencies, option) {
    ...
    return generateModuleInfo(moduleInfo, moduleName, modulePath, moduleConf, option);
...
const initOption = function(option) {
    return {
        ...
        overrides: {},
        dependencies: {},
        onDependencies: function(dependencies, _moduleConf, _option) {
            return dependencies;
        },
        onFiles: function(files, _moduleConf, _option) {
            return files;
        },
        ... option,
        moduleConf: {},
...
const flatdep = function(option) {
    ...
    if (moduleConf.browser) {
        option.overrides = {
            ... moduleConf.browser, ... option.overrides
        };
    }
